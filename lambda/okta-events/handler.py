...
Okta Event Hook Handler — automatic offboarding

When a user is deactivated/suspended in Okta or unassigned from the app,
immediately revoke their LiteLLM Virtual Key. (Alternative implementation of SCIM deprovisioning)

Processing flow:
1. GET  + X-Okta-Verification-Challenge header -> one-time verification response (once, at Okta hook registration)
2. POST -> compare the Authorization header against the webhook secret (401 on mismatch)
3. Event parsing:
   - user.lifecycle.deactivate / suspend / delete.initiated -> always revoke
   - application.user_membership.remove -> revoke only when the app matches OKTA_APP_LABEL
4. Revoke = delete LiteLLM keys (/key/delete) + delete DynamoDB cache
   (the LiteLLM user record is kept for usage auditing)
"""
...
# User-level deprovisioning events (revoke immediately regardless of app)
...
# App unassignment event (revoke only when OKTA_APP_LABEL matches)
APP_UNASSIGN_EVENT = "application.user_membership.remove"
# 그룹 제거 이벤트 (OKTA_GROUP_LABEL 일치 시에만 회수 — 그룹 기반 어사인 운영용)
# Group removal event (revoke only when OKTA_GROUP_LABEL matches — for group-based assignment operations)
...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    API Gateway (REST proxy) handler.
    """
    try:
        ...
        # 1. Okta one-time verification (challenge sent via GET at hook registration)
        if method == "GET":
        ...
        # Return 200 to prevent Okta retries (errors are tracked via logs)
        return _json_response(200, {"error": "internal error, logged"})


# ---------------------------------------------------------------------------
# 이벤트 파싱 / Event parsing
# ---------------------------------------------------------------------------

def _extract_target_email(e: dict[str, Any]) -> str | None:
    """회수 대상 이벤트면 대상 사용자의 이메일을 반환, 아니면 None.

    Return the target user's email if this is a revocation-target event, otherwise None.
    """
    ...
    if event_type == APP_UNASSIGN_EVENT:
        ...
        if not expected_label:
            ...
            # If the app filter is unset, ignore to avoid mistakenly revoking on unrelated app unassignments
    ...
    if event_type == GROUP_UNASSIGN_EVENT:
        ...
        # Supports multiple comma-separated groups. If unset, ignore to avoid mistakenly revoking on unrelated group removals.
...
# ---------------------------------------------------------------------------
# 접근 회수 / Access revocation
# ---------------------------------------------------------------------------

def _revoke_user_access(email: str) -> None:
    """LiteLLM 키 전부 삭제 + DynamoDB 캐시 삭제. 사용자 레코드는 감사용으로 유지.

    Delete all LiteLLM keys + delete the DynamoDB cache. The user record is kept for auditing.
    """
    master_key = _get_master_key()

    # 1. 사용자의 모든 키 조회 후 삭제 (alias 규칙에 의존하지 않음)
    # 1. Look up all of the user's keys and delete them (does not rely on alias conventions)
    ...
    # 2. Delete the DynamoDB cache (prevents key reissue on next login — re-login itself is blocked by Okta)
...
def _collect_user_key_tokens(master_key: str, email: str) -> list[str]:
    ...
    Collect all of the user's key tokens.

    /user/info is the primary path. For legacy cases where keys exist without a
    user record, fall back to /key/list?user_id= on 404.
    """
...
def _litellm_request(method: str, path: str, master_key: str, body: dict | None = None) -> dict:
    ...
    # 자체서명 인증서 ALB 대응 / Handle ALB with a self-signed certificate
